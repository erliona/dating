...
async def list_admins():
    ...
    async with session_maker() as session:
        ...
        print(f"\n{'='*80}")
        print(f"{'ID':<5} {'Username':<20} {'Full Name':<25} {'Super Admin':<12} {'Active'}")
        print(f"{'='*80}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Использование:")
        print("  python scripts/create_admin.py create <username> <password> [full_name] [email] [--super-admin]")
        print("  python scripts/create_admin.py list")
        print("  python scripts/create_admin.py change-password <username> <new_password>")
        print("\nПримеры:")
        print("  python scripts/create_admin.py create admin admin123")
        print("  python scripts/create_admin.py create john secretpass 'John Doe' john@example.com --super-admin")
        print("  python scripts/create_admin.py list")
    ...
    if command == "create":
        if len(sys.argv) < 4:
            ...
            print("Использование: create <username> <password> [full_name] [email] [--super-admin]")
            return
        
        username = sys.argv[2]
        password = sys.argv[3]
        full_name = sys.argv[4] if len(sys.argv) > 4 and not sys.argv[4].startswith('--') else None
        email = sys.argv[5] if len(sys.argv) > 5 and not sys.argv[5].startswith('--') else None
