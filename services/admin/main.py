...
async def login_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        async with session_maker() as session:
            ...
            token = await create_session_token(admin.id, request.app["config"]["jwt_secret"])
...
async def get_stats_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        async with session_maker() as session:
            ...
            # Total profiles
            total_profiles_result = await session.execute(select(func.count(Profile.id)))
            ...
            # Total interactions
            total_interactions_result = await session.execute(select(func.count(Interaction.id)))
...
def create_app(config: dict) -> web.Application:
    ...
    app.router.add_static('/admin-panel/', path=os.path.join(os.path.dirname(__file__), 'static'), name='static')
