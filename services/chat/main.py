        ...
        async with session.request(method, url, json=data, headers=headers) as resp:
            if resp.status >= 400:
                raise ExternalServiceError(
                    f"Data service error: {resp.status}", "External service call failed"
                )
...
async def websocket_handler(request: web.Request):
    ...
    logger.info(
        f"WebSocket connection established for user {user_id}, conversation {conversation_id}"
    )
...
async def handle_message_event(ws, data, user_id, conversation_id):
    ...
    try:
        result = await _call_data_service(
            ...
            "POST",
            {"sender_id": user_id, "content": content, "content_type": content_type},
        )
...
async def handle_typing_event(ws, data, user_id, conversation_id):
    ...
    await ws.send_json(
        {"type": "conversation.typing", "user_id": user_id, "is_typing": is_typing}
    )
...
async def get_conversations(request: web.Request) -> web.Response:
    ...
    try:
        ...
        with_unread_only = (
            request.query.get("with_unread_only", "false").lower() == "true"
        )
...
async def update_read_state(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not up_to_message_id:
            return web.json_response(
                {"error": "up_to_message_id is required"}, status=400
            )
...
async def block_user(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not target_user_id:
            return web.json_response(
                {"error": "target_user_id is required"}, status=400
            )
        ...
        if event_publisher:
            await event_publisher.publish_event(
                "user.blocked", {"blocker_id": user_id, "blocked_id": target_user_id}
            )
...
async def create_report(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if not conversation_id or not reason:
            return web.json_response(
                {"error": "conversation_id and reason are required"}, status=400
            )
...
def create_app(config: dict) -> web.Application:
    ...
    app.router.add_get(
        "/chat/conversations/{conversation_id}/messages", get_messages
    )  # Fixed route
    app.router.add_post(
        "/chat/conversations/{conversation_id}/messages", send_message
    )  # Fixed: messages belong to conversations
    app.router.add_put(
        "/chat/conversations/{conversation_id}/read-state", update_read_state
    )  # Fixed: read-state per conversation
    app.router.add_post(
        "/chat/blocks", block_user
    )  # Fixed: blocks are user-to-user relationships
    app.router.add_delete("/chat/blocks/{target_user_id}", unblock_user)  # Unblock user
    app.router.add_post(
        "/chat/reports", create_report
    )  # Fixed: reports are separate resources
