...
from sqlalchemy import Boolean, Column, DateTime, String, Time, func
...
class NotificationPreferences(Base):  # type: ignore[misc,valid-type]
    ...
    def is_quiet_hours(self, current_time: time | None = None) -> bool:  # type: ignore[return-value]
        ...
        if self.quiet_hours_start <= self.quiet_hours_end:
            return bool(self.quiet_hours_start <= current_time <= self.quiet_hours_end)
        else:
            return bool(
    ...
    def should_send_notification(self, notification_type: str) -> bool:
        ...
        return type_mapping.get(notification_type, True)  # type: ignore[return-value]
