  ...
  useEffect(() => {
    const authenticateWithTelegram = async () => {
      try {
        ...
        addDebugInfo(`Cookies after auth: ${cookiesAfter || 'none'}`);
        
        // Check for access token specifically
        const accessTokenAfter = document.cookie
          .split("; ")
          .find((row) => row.startsWith("access_token="))
          ?.split("=")[1];
        addDebugInfo(`Access token after auth: ${accessTokenAfter ? 'present' : 'missing'}`);
        ...
        setTimeout(() => {
          addDebugInfo("Redirecting to welcome page...");
          router.push("/ru/welcome");
        }, 1000);
      } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          {/* Debug info */}
          <div className="text-xs text-left bg-gray-100 p-3 rounded max-h-40 overflow-y-auto mb-6">
            <div className="font-semibold mb-2">Debug Info:</div>
            {debugInfo.length > 0 ? (
              debugInfo.map((info, index) => (
                <div key={index} className="mb-1 text-gray-600">{info}</div>
