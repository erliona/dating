...
def test_loki_config_no_deprecated_fields():
    ...
    assert 'max_transfer_retries' not in content, "Should not have deprecated max_transfer_retries"
    assert 'enforce_metric_name' not in content, "Should not have deprecated enforce_metric_name"
...
def test_loki_config_has_compactor():
    ...
    assert config['compactor']['retention_enabled'] is True, "Retention should be enabled"
    ...
    for dashboard_file in dashboard_dir.glob("*.json"):
        with open(dashboard_file, 'r') as f:
            try:
                ...
                assert dashboard is not None, f"{dashboard_file.name} should be valid JSON"
...
def test_docker_compose_has_version_pinning():
    ...
    assert 'grafana/promtail:latest' not in content, "Promtail should not use :latest tag"
    assert 'grafana/grafana:latest' not in content, "Grafana should not use :latest tag"
    assert 'prom/prometheus:latest' not in content, "Prometheus should not use :latest tag"
...
def test_docker_compose_has_env_expansion_flag():
    ...
    assert '-config.expand-env=true' in content, "Should have config.expand-env flag for Loki"
...
def test_docker_compose_has_health_checks():
    ...
    for service_name in services_to_check:
        if service_name in services:
            ...
            assert 'test' in service['healthcheck'], f"{service_name} healthcheck should have a test"
...
def test_monitoring_documentation_updated():
    ...
    assert 'v2.51.0' in content or '2.51.0' in content, "Should mention Prometheus version"
