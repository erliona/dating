    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if data.get("status") == "ok":
                ...
                print(f"ERROR: HTTP API returned unexpected status: {data}", file=sys.stderr)
                return False
                
    except urllib.error.HTTPError as e:
        print(f"WARNING: HTTP error from local API: {e.code} {e.reason}", file=sys.stderr)
        return False
...
def check_bot_health() -> bool:
    ...
    print("Local HTTP API not available, falling back to Telegram API check", file=sys.stderr)
