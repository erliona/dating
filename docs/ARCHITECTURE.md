...
Dating App - это микросервисная архитектура с независимым ядром, построенная на Docker контейнерах с полной автоматизацией CI/CD.

### Принципы архитектуры

1. **Платформонезависимое ядро** - вся бизнес-логика полностью независима от платформы (Telegram, mobile, web)
2. **Микросервисы** - приложение разделено на независимые сервисы для масштабируемости
3. **Адаптеры** - платформенная интеграция через адаптеры (Telegram, mobile apps, etc.)
4. **API-first** - все сервисы взаимодействуют через REST API
...
### 5. Legacy Bot (`/bot`)
**Telegram-специфичный код (постепенно мигрируется)**

Код, который еще не перенесен в ядро или микросервисы.

## Компоненты системы (Legacy)
...
- Rate limiting with automatic TTL cleanup
- Session refresh functionality
- Security event logging

#### `bot/cache.py`
- In-memory кэширование профилей
- TTL управление с автоматической очисткой
...
- Проверка возраста (18-120 лет)
- Валидация полей формы
- Консистентная валидация между frontend и backend
