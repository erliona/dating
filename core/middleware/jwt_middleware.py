...
from core.middleware.security_metrics import record_auth_failure, record_jwt_validation
from core.utils.security import ValidationError, validate_jwt_token
...
async def jwt_middleware(request: web.Request, handler) -> web.Response:
    ...
    if (
        ...
        or request.path.startswith("/sync-metrics")
    ):
        return await handler(request)

    # Пропустить auth endpoints (кроме verify)
    if request.path.startswith("/auth/") and not request.path.startswith(
        "/auth/verify"
    ):
        return await handler(request)
    ...
    try:
        ...
        if not user_id:
            ...
            return web.json_response(
                {"error": "Invalid token: missing user_id"}, status=401
            )
...
async def admin_jwt_middleware(request: web.Request, handler) -> web.Response:
    ...
    if not auth_header or not auth_header.startswith("Bearer "):
        logger.warning(
            f"Missing Authorization header for admin endpoint {request.path}"
        )
        return web.json_response(
            {"error": "Missing or invalid Authorization header"}, status=401
        )
    ...
    try:
        ...
        if not admin_id:
            ...
            return web.json_response(
                {"error": "Invalid admin token: missing admin_id"}, status=401
            )
        ...
        return web.json_response({"error": f"Invalid admin token: {e}"}, status=401)
    except Exception as e:
        logger.error(f"Admin JWT middleware error: {e}")
        return web.json_response({"error": "Admin authentication error"}, status=500)
