...
SERVICE_RATE_LIMITERS = {
    ...
    "notification-service": RateLimiter(max_requests=50, window_seconds=60),  # 50 req/min
    "data-service": RateLimiter(max_requests=100, window_seconds=60),  # 100 req/min (internal)
}
...
ENDPOINT_RATE_LIMITERS = {
    ...
    "/media/upload/photo": RateLimiter(max_requests=3, window_seconds=60),  # 3 photos/min
    
    # Discovery endpoints - swipe limits
    "/discovery/swipe": RateLimiter(max_requests=30, window_seconds=60),  # 30 swipes/min
    "/discovery/swipe": RateLimiter(max_requests=30, window_seconds=60),  # 30 swipes/min
    
    # Chat endpoints - message limits
    "/chat/messages": RateLimiter(max_requests=20, window_seconds=60),  # 20 messages/min
    "/chat/conversations/{id}/messages": RateLimiter(max_requests=20, window_seconds=60),
    
    # Admin endpoints - moderation limits
    "/admin/moderation": RateLimiter(max_requests=10, window_seconds=60),  # 10 actions/min
...
def get_rate_limiter_for_service(service_name: str) -> RateLimiter:
    ...
    return SERVICE_RATE_LIMITERS.get(service_name, RateLimiter(max_requests=50, window_seconds=60))
...
@web.middleware
async def service_rate_limiting_middleware(request: web.Request, handler) -> web.Response:
    """
    ...
    if endpoint_limiter:
        if not endpoint_limiter.is_allowed(user_id):
            ...
            record_rate_limit_hit(service_name, request.path, user_id, "endpoint")
            ...
            }, status=429, headers={'Retry-After': str(endpoint_limiter.window_seconds)})
    ...
    if not service_limiter.is_allowed(user_id):
        ...
        record_rate_limit_hit(service_name, request.path, user_id, "service")
        ...
        }, status=429, headers={'Retry-After': str(service_limiter.window_seconds)})
...
class AuthRateLimiter:
    ...
    def __init__(self):
        self.login_limiter = RateLimiter(max_requests=5, window_seconds=60)  # 5 login attempts/min
        self.refresh_limiter = RateLimiter(max_requests=10, window_seconds=60)  # 10 refresh/min
        self.validate_limiter = RateLimiter(max_requests=5, window_seconds=60)  # 5 validate/min
...
class MediaRateLimiter:
    ...
    def __init__(self):
        self.upload_limiter = RateLimiter(max_requests=3, window_seconds=60)  # 3 uploads/min
        self.photo_limiter = RateLimiter(max_requests=2, window_seconds=60)  # 2 photos/min
        self.video_limiter = RateLimiter(max_requests=1, window_seconds=60)  # 1 video/min
...
class DiscoveryRateLimiter:
    ...
    def __init__(self):
        self.swipe_limiter = RateLimiter(max_requests=30, window_seconds=60)  # 30 swipes/min
        self.super_like_limiter = RateLimiter(max_requests=5, window_seconds=60)  # 5 super likes/min
        self.undo_limiter = RateLimiter(max_requests=3, window_seconds=300)  # 3 undos/5min
...
class ChatRateLimiter:
    ...
    def __init__(self):
        self.message_limiter = RateLimiter(max_requests=20, window_seconds=60)  # 20 messages/min
        self.typing_limiter = RateLimiter(max_requests=10, window_seconds=60)  # 10 typing events/min
        self.read_limiter = RateLimiter(max_requests=50, window_seconds=60)  # 50 read updates/min
