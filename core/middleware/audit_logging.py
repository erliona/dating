...
def audit_log(
    ...
    **kwargs,
    ...
    if details:
        audit_entry["details"] = details  # type: ignore[assignment]  # type: ignore[assignment]

    # Add any additional fields
    audit_entry.update(kwargs)

    # Log the audit entry
    logger.log(log_level, f"AUDIT: {operation} by user {user_id}", extra=audit_entry)
...
def _determine_operation(request: web.Request) -> str | None:
...
def log_security_event(
    ...
    **kwargs,
    ...
    if details:
        audit_entry["details"] = details  # type: ignore[assignment]

    audit_entry.update(kwargs)

    log_level = getattr(logging, severity, logging.WARNING)
    logger.log(log_level, f"SECURITY AUDIT: {event_type}", extra=audit_entry)
