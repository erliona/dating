...
class ServiceCircuitBreaker:
    ...
    def __init__(
        ...
        self.breaker = CircuitBreaker(
            fail_max=fail_max, timeout_duration=timeout_duration, name=service_name
        )
    ...
    async def call(
        ...
        try:
            ...
            if isinstance(e, CircuitBreakerError) or (
                hasattr(self, "expected_exception")
            ...
            CIRCUIT_BREAKER_STATE.labels(
                service=self.service_name, target=self.service_name
            ).set(current_state.value)


# Global circuit breakers for each service
data_service_breaker = ServiceCircuitBreaker(
    "data-service", fail_max=5, timeout_duration=60
)
bot_service_breaker = ServiceCircuitBreaker(
    "telegram-bot", fail_max=3, timeout_duration=30
)
