...
def validate_username(username: str) -> str:
    ...
    if not re.match(r'^[a-zA-Z0-9_]{5,32}$', username):
        raise ValidationError("Username must be 5-32 characters, alphanumeric and underscores only")
...
def validate_profile_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "height" in data:
        validated["height"] = validate_height(data["height"])
    
    if "weight" in data:
        validated["weight"] = validate_weight(data["weight"])
    ...
    if "has_children" in data:
        validated["has_children"] = validate_boolean(data["has_children"], "has_children")
