...
def optimize_image(image_bytes: bytes, format: str = "JPEG", quality: int = 85) -> bytes:
...
async def check_profile_handler(request):
    ...
    if not user_id:
        return web.json_response(
            {"error": {"code": "validation_error", "message": "Missing user_id parameter"}}, status=400
        )
    ...
    try:
        ...
        return web.json_response(
            {"error": {"code": "validation_error", "message": "Invalid user_id format - must be integer"}}, status=400
        )
