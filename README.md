...
- `WEBAPP_URL` — публичный URL мини-приложения. По умолчанию указывает на локальный
  контейнер Nginx. В Docker Compose статическое приложение отдаётся из корня
  (`/`), поэтому добавление дополнительного сегмента пути приведёт к ошибке 404.
...
1. Установите Python 3.11+, PostgreSQL и создайте базу данных.
   ...
   psql -U postgres -c "CREATE USER dating WITH PASSWORD 'dating';"
   psql -U postgres -c "CREATE DATABASE dating OWNER dating;"
   ```
...
3. Установите переменные окружения:
   ...
   export BOT_DATABASE_URL="postgresql+asyncpg://dating:dating@localhost:5432/dating"
   export WEBAPP_URL="https://example.com/"
   ```
   
   **Важно:** Убедитесь, что пользователь `dating`, пароль `dating` и база данных 
   `dating` существуют в PostgreSQL и совпадают со строкой подключения.
...
### Важно: Синхронизация переменных окружения

При запуске **с Docker Compose**:
- `BOT_DATABASE_URL` формируется автоматически из переменных `POSTGRES_USER`, 
  `POSTGRES_PASSWORD`, `POSTGRES_DB` в файле `.env`
- Не нужно задавать `BOT_DATABASE_URL` вручную

При запуске **без Docker** (локально):
- Необходимо **явно указать** `BOT_DATABASE_URL` в `.env` или экспортировать как 
  переменную окружения
- Формат: `postgresql+asyncpg://user:password@host:port/database`
- Пользователь, пароль и база данных **должны существовать** в PostgreSQL

### Устранение ошибки "password authentication failed"

Если при запуске или сохранении анкеты появляется ошибка:
```
password authentication failed for user "dating"
```
...
2. **Проверьте, что пользователь и база данных созданы:**
   ```bash
   # Подключитесь к PostgreSQL от имени суперпользователя
   psql -U postgres
   
   # Создайте пользователя (если не существует)
   CREATE USER dating WITH PASSWORD 'dating';
   
   # Создайте базу данных (если не существует)
   CREATE DATABASE dating OWNER dating;
   
   # Дайте права пользователю
   GRANT ALL PRIVILEGES ON DATABASE dating TO dating;
   ```

3. **Проверьте переменные окружения:**
   ```bash
   # Для запуска без Docker
   echo $BOT_DATABASE_URL
   # Должно быть примерно: postgresql+asyncpg://dating:dating@localhost:5432/dating
   
   # Убедитесь, что логин/пароль в строке совпадают с PostgreSQL
   ```

4. **Проверьте файл `.env`:**
   - При использовании Docker Compose убедитесь, что `POSTGRES_USER`, 
     `POSTGRES_PASSWORD` и `POSTGRES_DB` заданы корректно
   - При локальном запуске убедитесь, что `BOT_DATABASE_URL` раскомментирован и 
     содержит правильные учётные данные

5. **Проверьте права доступа в PostgreSQL:**
   ```bash
   # Посмотрите настройки аутентификации
   cat /etc/postgresql/*/main/pg_hba.conf
   
   # Для локальной разработки используйте метод 'md5' или 'scram-sha-256'
   # local   all   dating   md5
   # host    all   dating   127.0.0.1/32   md5
   ```
...
Для разработки предусмотрены автоматические тесты, покрывающие бизнес-логику бота и
уровень работы с базой данных. Чтобы запустить их локально:
...
Тесты используют встроенную SQLite в памяти, поэтому не требуют запуска отдельных
сервисов. При успешном прохождении вы увидите отчёт Pytest без ошибок и предупреждений.
