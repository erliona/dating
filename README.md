...
**Проблема:** Ошибка `BOT_TOKEN environment variable is required`
- **Решение:** Убедитесь, что переменная окружения `BOT_TOKEN` установлена и содержит валидный токен Telegram-бота. Проверьте файл `.env` или переменные окружения системы.

**Проблема:** Ошибка `BOT_DATABASE_URL must be a valid SQLAlchemy connection string`
- **Решение:** Проверьте формат строки подключения к базе данных. Она должна быть в формате `postgresql+asyncpg://user:password@host:port/database`. Пример: `postgresql+asyncpg://dating_user:mypassword@localhost:5432/dating`

**Проблема:** Ошибка `Only PostgreSQL databases are supported`
- **Решение:** Убедитесь, что используете PostgreSQL. SQLite и другие базы данных не поддерживаются для продакшн-использования.

### Проблемы с подключением к базе данных

**Проблема:** Ошибка `Cannot connect to database`
- **Решение:** 
  - Проверьте, что PostgreSQL запущен: `docker compose ps` (для Docker) или `systemctl status postgresql` (для Linux)
  - Убедитесь, что хост, порт, имя пользователя и пароль в `BOT_DATABASE_URL` корректны
  - Проверьте, что база данных создана: `psql -U postgres -c "\l"` и найдите вашу базу в списке
  - Убедитесь, что пользователь имеет права доступа к базе данных

**Проблема:** Миграции не применяются автоматически
- **Решение:** 
  - Проверьте переменную `RUN_DB_MIGRATIONS` в `.env` - она должна быть не установлена или иметь значение отличное от `false`
  - Примените миграции вручную: `alembic upgrade head`
  - Проверьте логи контейнера: `docker compose logs bot`

### Проблемы с WebApp

**Проблема:** Кнопка "Открыть мини-приложение" не появляется
- **Решение:** Убедитесь, что переменная `WEBAPP_URL` установлена в `.env` и содержит корректный публичный URL вашего веб-приложения.

**Проблема:** WebApp возвращает ошибку 404
- **Решение:** 
  - Для Docker Compose: Убедитесь, что `WEBAPP_URL` указывает на корневой путь без дополнительных сегментов (например, `http://localhost:8080/` а не `http://localhost:8080/app/`)
  - Проверьте, что контейнер `webapp` запущен: `docker compose ps webapp`

### Общие проблемы

**Проблема:** Бот не отвечает на сообщения
- **Решение:**
  - Проверьте логи бота: `docker compose logs -f bot` или запустите напрямую для просмотра вывода
  - Убедитесь, что токен бота валидный - проверьте через [@BotFather](https://t.me/BotFather)
  - Убедитесь, что бот запущен: `docker compose ps` или `ps aux | grep python`

**Проблема:** Профили не сохраняются
- **Решение:**
  - Проверьте подключение к базе данных (см. выше)
  - Проверьте логи на наличие ошибок SQLAlchemy
  - Убедитесь, что таблицы созданы: `alembic current` должен показать последнюю ревизию

**Проблема:** Ошибки при деплое через GitHub Actions
- **Решение:**
  - Проверьте, что все секреты (`DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY`, `DEPLOY_PATH`, `ENV_FILE`) установлены в настройках репозитория
  - Убедитесь, что SSH-ключ правильно добавлен на сервер в `~/.ssh/authorized_keys`
  - Проверьте, что пользователь на сервере имеет права на Docker (добавьте в группу `docker`)
