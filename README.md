...
- 🧪 **Тестирование** - 310+ тестов с высоким покрытием
...
```
┌─────────────────┐
│  Telegram Bot   │ ← Тонкий клиент, работает через API Gateway
└────────┬────────┘
         │ HTTP/REST
┌────────▼────────┐
...
- ✅ **Бот как тонкий клиент** - не имеет прямого доступа к БД
- ✅ **Вся бизнес-логика в микросервисах** - централизованная обработка
...
- Адаптивный дизайн с Safe Area поддержкой
- Без фреймворков для минимального размера
